...
function _safe_extrema(data::AbstractArray)
...
function _safe_mean_std(data::AbstractArray)
    valid_data = filter(!isnan, data)
    if isempty(valid_data)
...
function _minmax_normalize_to_01(data::AbstractArray)
    min_val, max_val = _safe_extrema(data)
